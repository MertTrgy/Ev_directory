...
def _parse_time_min(s: Any) -> Optional[int]:
    ...
    return total if total > 0 else _i(s)
...
def _map_vehicle(ev: Dict[str, Any], source_url: str) -> Dict[str, Any]:
    ...
    chg = _label_map(ev.get("charge_details"))
    ...
    real_miles = _f(typical.get("maxValue"))
    ...
    motorway_min = _parse_time_min(_get(chg, "motorway"))
...
def scrape_all_vehicles(
    ...
    progress.update({"running": True, "total": 0, "done": 0, "errors": 0})
