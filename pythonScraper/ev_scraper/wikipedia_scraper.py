...
def normalize_year(value: str) -> int:
    """Extracts the first 4-digit year from a text cell."""
...
def extract_headers(table) -> List[str]:
    """Returns normalized table headers for column matching."""
...
def find_index(headers: List[str], candidates: List[str]) -> int:
    """Finds the first header containing one of the candidate terms."""
...
def split_brand_model(vehicle: str, manufacturer: Optional[str]) -> Tuple[str, str]:
    """Splits the combined vehicle name into brand and model values."""
...
def to_item(brand: str, model: str, year: int) -> Dict[str, object]:
    """Builds the lightweight payload returned by the legacy scraper endpoint."""
...
def scrape_vehicles() -> List[Dict[str, object]]:
    """Scrapes EV rows from the Wikipedia production-vehicles list."""
    try:
